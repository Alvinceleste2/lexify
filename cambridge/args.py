...
def parse_args(session):
    ...
    # Add sub-command s
    parser_sw = sub_parsers.add_parser(
        "s", help="look up words/phrases; hidden for convenience, no need to type"
    )
    ...
    if len(sys.argv) == 1:
        ...
        for index, word in enumerate(argv_list):
            ...
            elif (word == "--search" or word == "-s") and (
                index == len(argv_list) - 1 or argv_list[index + 1][0] == "-"
            ):
                continue
            elif (word == "--webster" or word == "-w") and (
                index == len(argv_list) - 1 or argv_list[index + 1][0] == "-"
            ):
                continue
            elif (word == "--chinese" or word == "-c") and (
                index == len(argv_list) - 1 or argv_list[index + 1][0] == "-"
            ):
                continue
            else:
...
async def list_words(args):
    ...
    select_word = (
        get_cache_selection_by_fzf(data)
        if has_fzf
        else get_cache_selection(data, method)
    )
    ...
    if args.search:
        ...
        for w in cambridge_words.split(","):
            ...
            if i:
                tasks.append(
                    search_cambridge(
                        args.session, i, args.fresh, False, args.nosuggestions, None
                    )
                )
    if args.webster:
        ...
        for w in webster_words.split(","):
            ...
            if i:
                tasks.append(
                    search_webster(
                        args.session, i, args.fresh, args.nosuggestions, None
                    )
                )
    if args.chinese:
        ...
        for w in words.split(","):
            ...
            if i:
                tasks.append(
                    search_cambridge(
                        args.session, i, args.fresh, True, args.nosuggestions, None
                    )
                )
